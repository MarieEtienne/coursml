...
sim_tstat <- function(mu1, mu2, sigma, n1, n2){ 
  ...
  S2 <-  ( sum ((Y1k - Y1point)^2 ) + sum ( (Y2k - Y2point)^2 ) ) / (n1 + n2 - 2)
  S <- sqrt(S2)
  
  Tstat <- sqrt(n1+n2)*(Y1point - Y2point) / S
...
n_sim <- 100
df <- data.frame(tstat = replicate(n_sim, sim_tstat(mu1, mu2, sigma, 10, 15)), scenario = rep('S1', n_sim))

ggplot(df, aes(x=tstat, fill = scenario)) + geom_histogram(bins = sqrt(n_sim))
